    ...
    public void loadMenu() {
        for (Item item : ItemManager.INSTANCE.getAvailableItems()) {
            if(item instanceof BlockItem) {
                inventory.setItem(10, createGuiItem(item.getIcon().getType(), "&f&l&o"+item.getName(), "&8Block Pack", "", "&7Cost: &6" + item.getPrice() + " coins", "", "&aClick to buy!"));
            } else if(item instanceof HatItem) {
                inventory.setItem(11, createGuiItem(item.getIcon().getType(), "&f&l&o" + item.getName(), "&8Helmet", "", "&7Cost: &6" + item.getPrice() + " coins", "", "&aClick to buy!"));
            } else if(item instanceof StickItem) {
                inventory.setItem(12, createGuiItem(item.getIcon().getType(), "&f&l&o" + item.getName(), "&8Stick", "", "&7Cost: &6" + item.getPrice() + " coins", "", "&aClick to buy!"));
            }
        }
